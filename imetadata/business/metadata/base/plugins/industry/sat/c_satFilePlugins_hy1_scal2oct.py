...
class CSatFilePlugins_hy1_scal2oct(CSatFilePlugins_hy1):

    def get_information(self) -> dict:
        information = super().get_information()
        information[self.Plugins_Info_Type] = 'HY1_SCAL2OCT'
        return information
    ...
    def get_classified_character_of_sat(self, sat_file_status):
        ...
        if (sat_file_status == self.Sat_Object_Status_Zip) or (sat_file_status == self.Sat_Object_Status_Dir):
            return r'(?i)^H1C_OPER_OCT_L2.*', self.TextMatchType_Regex
        else:
            return r'(?i)^H1C_OPER_OCT_L2.*[.]tiff$', self.TextMatchType_Regex

    def init_qa_file_list(self, parser: CMetaDataParser) -> list:
        return [
            {
                self.Name_FileName: self.get_fuzzy_metadata_file(r'(?i)^H1C_OPER_OCT_L2.*[.]tiff$',

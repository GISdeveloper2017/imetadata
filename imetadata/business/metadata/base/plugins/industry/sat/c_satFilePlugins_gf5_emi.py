...
class CSatFilePlugins_gf5_emi(CSatFilePlugins_gf5):
    def get_information(self) -> dict:
        ...
        information[self.Plugins_Info_Type] = 'GF5_EMI'
        information[self.Plugins_Info_Type_Title] = '高分五号EMI传感器'
        return information

    def get_classified_character_of_sat(self, sat_file_status):
        if (sat_file_status == self.Sat_Object_Status_Zip) or (sat_file_status == self.Sat_Object_Status_Dir):
            return r'(?i)^GF5.*EMI.*_.*', self.TextMatchType_Regex
        else:
            # 散列文件的识别方式后续有待开发，目前暂不测试
            return r'(?i)^GF5.*EMI.*[.]tiff$', self.TextMatchType_Regex
    ...
    def init_qa_file_list(self, parser: CMetaDataParser) -> list:
        return [
            {
                # 主文件质检有待调整
                self.Name_FileName: self.get_fuzzy_metadata_file(r'(?i)^GF5.*EMI.*[.]tiff$',
    ...
    def get_metadata_bus_configuration_list(self) -> list:
        ...
        return [
            ...
            },
            {
                ...
                self.Name_XPath: '/ProductMetaData/SampleLat'
            },
            {
                self.Name_ID: 'centerlongitude',  # 中心经度
                self.Name_XPath: '/ProductMetaData/SampleLong'
            },
            {
            ...
            },
            {
                ...
                self.Name_XPath: '/ProductMetaData/NadirPixelSize'
            },
            {
        ...
        return [
            {
                ...
                self.Name_FileName: self.get_fuzzy_metadata_file(r'(?i)GF5.EMI.*.jpg',
                                                                 '{0}.jpg'.format(

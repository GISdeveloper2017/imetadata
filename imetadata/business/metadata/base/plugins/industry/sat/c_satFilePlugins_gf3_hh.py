...
class CSatFilePlugins_gf3_hh(CSatFilePlugins_gf3):
    ...
    def get_classified_character_of_sat(self, sat_file_status):
        ...
        if (sat_file_status == self.Sat_Object_Status_Zip) or (sat_file_status == self.Sat_Object_Status_Dir):
            ...
            return r'(?i)GF3_.*_HH_.*_strip_0[.]tiff', self.TextMatchType_Regex

    def init_qa_file_list(self, parser: CMetaDataParser) -> list:
        return [
            {
                self.Name_FileName: self.get_fuzzy_metadata_file(r'(?i)GF3_.*_HH_.*_strip_0[.]tiff',
        ...
        return [
            {
                ...
                self.Name_FileName: self.get_fuzzy_metadata_file(
                    r'(?i)GF3.*HH(?!.*(thumb | thumnail)).*.jpg', '{0}_thumb.jpg'.format(self.classified_object_name())
                )
            },
            {

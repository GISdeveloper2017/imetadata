...
# @Author : 张源博
# @File : c_satFilePlugins_gf4_pmi.py
...
class CSatFilePlugins_gf4_pmi(CSatPlugins):
    def get_information(self) -> dict:
        ...
        information[self.Plugins_Info_Catalog] = '高分四号'
    ...
    def get_classified_character_of_sat(self, sat_file_status):
        ...
        if (sat_file_status == self.Sat_Object_Status_Zip) or (sat_file_status == self.Sat_Object_Status_Dir):
            return 'gf4_pmi_*_l1a*', self.TextMatchType_Common
        else:
            # GF4_PMS_E115.0_N36.6_20160803_L1A0000125813.tiff
            # 暂定 gf4_pms_*_l1a*.tiff为主对象文件
            return 'gf4_pms_*_l1a*.tiff', self.TextMatchType_Common
    ...
    def init_qa_file_list(self, parser: CMetaDataParser) -> list:
        ...
        return [
            ...
            {
                self.Name_FileName: '{0}.tiff'.format(self.classified_object_name().replace('PMI', 'IRS')),
                self.Name_ID: 'irs_tif',
                self.Name_Title: '影像文件irs',
                self.Name_Group: self.QA_Group_Data_Integrity,
                self.Name_Result: self.QA_Result_Error,
                self.Name_Format: self.DataFormat_Raster_File
            }
        ...
        return [
            {
                ...
                self.Name_FileName: '{0}.jpg'.format(self.classified_object_name().replace('_PMI_', '_PMS_'))

            },
            {
                self.Name_ID: self.View_MetaData_Type_Thumb,
                self.Name_FileName: '{0}.thumb.jpg'.format(self.classified_object_name().replace('_PMI_', '_PMS_'))
            }
        ]

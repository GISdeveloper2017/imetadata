...
from imetadata.business.metadata.base.plugins.industry.sat.base.c_satFilePlugins_gf2 import CSatFilePlugins_gf2


class CSatFilePlugins_gf2_pms(CSatFilePlugins_gf2):

    def get_information(self) -> dict:
        information = super().get_information()
        information[self.Plugins_Info_Type] = 'GF2_PMS'
        information[self.Plugins_Info_Type_Title] = '高分二号PMS传感器'
        return information
    ...
    def get_metadata_bus_filename_by_file(self) -> str:
        ...
        return CFile.join_file(
            ...
            self.get_fuzzy_metadata_file('.*PAN.*.xml', '{0}-PAN.xml'.format(self.classified_object_name())
                                         )
        )
        ...
        return [
            {
                ...
                self.Name_FileName: self.get_fuzzy_metadata_file('.*MSS(?!.*thumb).*.jpg',
                                                                 '{0}-MSS.jpg'.format(self.file_info.file_main_name))
            },
            {
                self.Name_ID: self.View_MetaData_Type_Thumb,
                self.Name_FileName: self.get_fuzzy_metadata_file('.*MSS.*_thumb.jpg',

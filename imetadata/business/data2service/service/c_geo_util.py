...
def GetProj4(data_path, force_to_df=False):
    ...
    if str(data_path).endswith('.shp'):
        ...
        prosrs = osr.SpatialReference()
        if imgsrs != None and imgsrs != '':
            prosrs.ImportFromWkt(imgsrs)
        proj4_res = prosrs.ExportToProj4()

        del imgds

    if proj4_res == "" and force_to_df:
        prosrs = osr.SpatialReference()
        prosrs.ImportFromEPSG(21480)
        proj4_res = prosrs.ExportToProj4()
...
def IsExtIntersect(ext1, ext2):
    if (ext2[0] < ext1[0] < ext2[1] or ext2[0] < ext1[1] < ext2[1]) and (
            ext2[2] < ext1[2] < ext2[3] or ext2[2] < ext1[3] < ext2[3]):
        return True
    return False

...
class CControlCenter(Process):
    ...
    def run(self):
        ...
        self.__control_center_objects__ = Manager().dict()
        self.__control_center_objects_locker__ = Lock()
        self.__sentinel_queue__ = Queue()
        self.__sentinel_stop_event__ = Event()
    ...
    def process_command(self, command):
        cmd_type = command[serviceConst.NAME_CMD_TYPE]
        cmd_title = command[serviceConst.NAME_CMD_TITLE]

        if MetaDataUtils.equal_ignore_case(cmd_type, serviceConst.CMD_START):
            Logger().info('控制中心进程[{0}]收到了启动调度的指令...'.format(self.pid))
            self.command_start(command)
        elif MetaDataUtils.equal_ignore_case(cmd_type, serviceConst.CMD_STOP):
            Logger().info('控制中心进程[{0}]收到了停止调度的指令...'.format(self.pid))
            self.command_stop(command)
        elif MetaDataUtils.equal_ignore_case(cmd_type, serviceConst.CMD_SPEED_UP):
            Logger().info('控制中心进程[{0}]收到了加速调度的指令...'.format(self.pid))
            time.sleep(3)
            print('service: {0} speed up'.format(cmd_title))
        elif MetaDataUtils.equal_ignore_case(cmd_type, serviceConst.CMD_SPEED_DOWN):
    ...
    def process_sentinel_message(self, sentinel_message):
        ...
        :return:
        """
        cmd_id = sentinel_message[serviceConst.NAME_CMD_ID]
        cmd_title = sentinel_message[serviceConst.NAME_CMD_TITLE]
        if cmd_id is None:
            return
    ...
    def command_start(self, command):
        ...
        cmd_title = command[serviceConst.NAME_CMD_TITLE]
        cmd_algorithm = command[serviceConst.NAME_CMD_ALGORITHM]
        cmd_trigger = command[serviceConst.NAME_CMD_TRIGGER]
        cmd_parallel_count = command[serviceConst.NAME_CMD_PARALLEL_COUNT]
        cmd_max_parallel_count = command[serviceConst.NAME_CMD_MAX_PARALLEL_COUNT]

        Logger().info('调度{0}.{1}.{2}将启动{3}个并行进程...'.format(cmd_id, cmd_title, cmd_algorithm, cmd_parallel_count))

        params = Manager().dict()

        params[serviceConst.NAME_CMD_ID] = command[serviceConst.NAME_CMD_ID]
        params[serviceConst.NAME_CMD_TITLE] = command[serviceConst.NAME_CMD_TITLE]
        params[serviceConst.NAME_CMD_ALGORITHM] = command[serviceConst.NAME_CMD_ALGORITHM]
        params[serviceConst.NAME_CMD_TRIGGER] = command[serviceConst.NAME_CMD_TRIGGER]
        params[serviceConst.NAME_CMD_PARALLEL_COUNT] = command[serviceConst.NAME_CMD_PARALLEL_COUNT]
        params[serviceConst.NAME_CMD_MAX_PARALLEL_COUNT] = command[serviceConst.NAME_CMD_MAX_PARALLEL_COUNT]
        ...
        control_center_object.params = command
        control_center_object.stop_event = Event()
        control_center_object.subprocess_list = Manager().list()

        for i in range(cmd_parallel_count):
            proc = CWorker(control_center_object.stop_event, control_center_object.params)
            proc.start()
            control_center_object.subprocess_list.append(proc)
    ...
    def before_stop(self):
        ...
        while len(self.__control_center_objects__) > 0:
            control_center_object = self.__control_center_objects__.popitem()
    ...
    def command_stop(self, command):
        ...
        cmd_title = params[serviceConst.NAME_CMD_TITLE]
        cmd_algorithm = params[serviceConst.NAME_CMD_ALGORITHM]

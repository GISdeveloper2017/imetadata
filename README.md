...
1. 用户确认开始入库时, 系统将按如下步骤完成入库工作:
    ...
    * 根据用户选中的情况, 将用户未选中的数据对象, 从核心存储中移出至原位置(注意: 需要重建子目录结构!)
    * 系统清理用户未选中的数据对象的元数据信息(从数据库中删除)
    * 将最后确认的入库记录的目录\文件可用性更新为"可用"
1. 采用上述技术方案的好处:
    1. 如果交换存储和核心存储采用同一个存储介质, 则数据入库的过程效率将极高
    1. 将整个数据目录移入核心存储, 然后再将不合格的数据移出:
        1. 有效数据可以立即使用
        1. 部分数据可能和识别出的对象关系不明确, 但在管理中, 不会随着无效数据移出, 以确保最大可入数据的存储
